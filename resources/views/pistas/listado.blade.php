...
<!-- Modal-->
<div class="modal fade" id="modalPista" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">FORMULARIO DE PISTA</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i aria-hidden="true" class="ki ki-close"></i>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ url('Pista/guarda') }}" method="POST" id="formulario-tipos">
                	@csrf
                	<div class="row">

                		<div class="col-md-4">
                			<div class="form-group">
                			    <label for="exampleInputPassword1">Nombre de la Pista
                			    <span class="text-danger">*</span></label>
                			    <input type="hidden" class="form-control" id="pista_id" name="pista_id" />
	...
	<div class="card card-custom gutter-b">
		<div class="card-header flex-wrap py-3">
			<div class="card-title">
				<h3 class="card-label">TIPOS DE PISTAS
				</h3>
			</div>
			<div class="card-toolbar">
				<!--begin::Button-->
				<a href="#" class="btn btn-primary font-weight-bolder" onclick="nuevo()">
					<i class="fa fa-plus-square"></i> NUEVA PISTA
				</a>
				<!--end::Button-->
			</div>
		</div>
		...
		<div class="card-body">
			...
			<div class="table-responsive m-t-40">
				<table class="table table-bordered table-hover table-striped" id="tabla-insumos">
					...
					<tbody>
						@forelse ($pistas as $pi)
							<tr>
								<td>{{ $pi->id }}</td>
								<td>{{ $pi->nombre }}</td>
								<td>{{ $pi->descripcion }}</td>
								<td>
									<button type="button" class="btn btn-sm btn-icon btn-warning" onclick="edita('{{ $pi->id }}', '{{ $pi->nombre }}', '{{ $pi->descripcion }}')">
										<i class="flaticon2-edit"></i>
									</button>
									<button type="button" class="btn btn-sm btn-icon btn-danger" onclick="elimina('{{ $pi->id }}', '{{ $pi->nombre }}')">
		...
		function elimina(id, nombre)
        {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    window.location.href = "{{ url('Pista/elimina') }}/"+id;
