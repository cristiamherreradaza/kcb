    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">FORMULARIO DE CATEGORIA DE PISTA</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i aria-hidden="true" class="ki ki-close"></i>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ url('CategoriasPista/guarda') }}" method="POST" id="formulario-tipos">
                	@csrf
                	<div class="row">

                		<div class="col-md-4">
                			<div class="form-group">
                			    <label for="exampleInputPassword1">Nombre de la Categoria Pista
	...
	<div class="card card-custom gutter-b">
		<div class="card-header flex-wrap py-3">
			<div class="card-title">
				<h3 class="card-label">TIPOS DE CATEGORIAS PISTAS
				</h3>
			</div>
			<div class="card-toolbar">
				<!--begin::Button-->
				<a href="#" class="btn btn-primary font-weight-bolder" onclick="nuevo()">
					<i class="fa fa-plus-square"></i> NUEVA CATEGORIA PISTA
				</a>
				<!--end::Button-->
			</div>
		</div>
		...
		<div class="card-body">
			...
			<div class="table-responsive m-t-40">
				<table class="table table-bordered table-hover table-striped" id="tabla-insumos">
					...
					<tbody>
						@forelse ($categoriaPista as $cp)
							<tr>
								<td>{{ $cp->id }}</td>
								<td>{{ $cp->nombre }}</td>
								<td>{{ $cp->descripcion }}</td>
								<td>
									<button type="button" class="btn btn-sm btn-icon btn-warning" onclick="edita('{{ $cp->id }}', '{{ $cp->nombre }}', '{{ $cp->descripcion }}')">
										<i class="flaticon2-edit"></i>
									</button>
									<button type="button" class="btn btn-sm btn-icon btn-danger" onclick="elimina('{{ $cp->id }}', '{{ $cp->nombre }}')">
										<i class="flaticon2-cross"></i>
									</button>
								</td>
							</tr>
						@empty
							<h3 class="text-danger">NO EXISTEN CATEGORIAS PISTAS</h3>
						@endforelse
					</tbody>
					<tbody>
					</tbody>
		...
		function elimina(id, nombre)
        {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    window.location.href = "{{ url('CategoriasPista/elimina') }}/"+id;
